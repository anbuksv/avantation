...
export const HOST: flages = {
  ...
  description: {
    short:
      "Filter the http request from HAR and use it as server url in output.",
    long:
...
export const BASE_PATH: flages = {
  ...
  description: {
    short:
      "Separate the common path as base path from HTTP requests. Example:['api/v1']",
...
export const PATH_REGEX: flages = {
  ...
  description: {
    ...
    long: `\tIn request URL regex matching path parameter will be converted into ${
      colors.Bright
    }${colors.fg.Green}dynamic path parameters${
      colors.Reset
...
export const SECURITY_HEADERS: flages = {
  ...
  description: {
    ...
    long: `\tTo map custom security headers link 'x-api-key' as autorization header.
            ...
            ${getTitle("Syntax")}
                {
                    "${colors.fg.Red}headerNameInHTTPRequest${
      colors.Reset
    }": {Sequrity Scheme Object as per OAS3.0}
                }
...
export const TEMPLATE: flages = {
  ...
  description: {
    short:
      "To override the default template pass the your template file location.",
    long:
      "\tYou can override the default template by passing your template file location."
  },
...
export const BUILD_STATIC_UI: flages = {
  ...
  description: {
    short:
      "Build the static user interface from generated OpenAPI3.0 specification."
  },
  default: `${getDefault("false")}`
};
...
function getExample() {
  return `  ${colors.fg.Green}avantation${colors.Reset} ${
    colors.fg.Yellow
  }--har${colors.Reset} example.com.har ${colors.fg.Yellow}-h${
    colors.Reset
  } apis.example.com ${colors.fg.Yellow}-b${colors.Reset} v1 ${
    colors.fg.Yellow
  }-o${colors.Reset} ./example.json`;
}

function getDefault(defaut: string): string {
  return `\t${colors.Bright}${colors.fg.Red}default:${colors.Reset} ${
    colors.Bright
  }${colors.fg.White}${defaut}${colors.Reset}`;
}
